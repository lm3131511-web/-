...
2. Run correlation pre-computation job (placeholder `_perform_cold_start()` hook in `codex/bootstrap.py`).
...
4. Execute `tests/stress/test_llm_api_schema_drift.py` to confirm schema stability.

## News Blackout
1. Confirm provider credentials and ingest health (`data/logs/news_ingest.jsonl`).
